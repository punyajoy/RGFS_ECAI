...
def transform_attention(exp_list):
    exp_list=[ele[0] for ele in exp_list]
...
def standaloneEval_with_rationale(model_to_use, dataset_name, test_data=None, topk=2, rational=False):
    ...
    all_probab,all_rationales=modelClass.return_probab(sentences_all)
    ...
    for index,row in tqdm(test_data.iterrows(),total=len(test_data)):
        ...
        pred_id=0
        max_pred=0
        for i,pred in enumerate(all_probab[index]):
...
#         reverse_dict={0:'normal',1:'hateful', 2: 'abusive'}
    ...
    all_probab,_=modelClass.return_probab(sentences_all)
    for index,row in tqdm(test_data.iterrows(),total=len(test_data)):
        temp={}
        pred_id=np.argmax(all_probab[index])
        pred_label=reverse_dict[pred_id]
...
def get_final_dict_with_lime(model_name, dataset_name, test_data, topk):
    list_dict_org,test_data=standaloneEval_with_rationale(model_name, dataset_name, test_data=test_data, topk=topk)
